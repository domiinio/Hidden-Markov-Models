...
def init_maze(maze_name, robot_pos_init=None):
...
def test_filtering():
    ...
    robot = init_maze('mazes/square.map', robot_pos_init=(1,1))
#    states, obs = robot.simulate(init_state=(1,1), n_steps=3)
    states, obs = robot.simulate(n_steps=300)
...
def test_smoothing():
    ...
    robot = init_maze('mazes/square.map', robot_pos_init=(1,1))
    states, obs = robot.simulate(init_state=(1,1), n_steps=300)
    print('Running smoothing...')
    initial_belief = normalized({pos: 1 for pos in robot.get_states()})
    beliefs = forwardbackward_scaling(initial_belief, obs, robot)
    probabilities = []
    for state, belief in zip(states, beliefs):
        for k, v in sorted(belief.items(), key=lambda x: x[1], reverse=True):
            if v > 0:
...
def test_viterbi():
    ...
    robot = init_maze('mazes/square.map', robot_pos_init=(1,1))
    states, obs = robot.simulate(init_state=(1, 1), n_steps=350)
    print('Running Viterbi...')
    initial_belief = normalized({pos: 1 for pos in robot.get_states()})
    ml_states, max_msgs = viterbi(initial_belief, obs, robot)
    max_probs = []
    for i in range(len(max_msgs)):
        max_msg_prob = max(list((max_msgs[i]).values()))
        max_probs.append(max_msg_prob)
    a = np.asarray([max_probs])
    np.savetxt("temp_result.csv", a.T, delimiter=";")
