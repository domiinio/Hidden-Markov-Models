...
class Robot(HMM):
    ...
    def get_transition_matrix(self):
        """ Returns matrix of transition probabilites """
        all_states = list(self.get_states())
        # prepare the transition matrix
        A = np.ndarray(shape=(len(all_states), len(all_states)), dtype = float)
        # fill the transition matrix with transition probabilities
        for i, j in product(range(len(all_states)), range(len(all_states))):
            A[i,j] = self.pt(all_states[i], all_states[j])
        return A.T

    def get_observation_matrix(self):
        """ Returns matrix of observation probabilities """
        all_states = list(self.get_states())
        all_observations = list(self.get_observations())
        # prepare the observation matrix
        B = np.ndarray(shape=(len(all_states), len(all_observations)), dtype=float)
        # fill the emission matrix with transition probabilities
        for i, j in product(range(len(all_states)), range(len(all_observations))):
            B[i,j] = self.pe(all_states[i], all_observations[j])
        return B